...
export class Game {
  ...
  isPointInsideRhombus(
    ...
    return (
      Math.abs(px - cx) / (shape.width / 2) +
        Math.abs(py - cy) / (shape.height / 2) <=
      1
    );
  }
  isNearPencilStroke(
    ...
    if (shape?.type !== "pencil" || !shape.points) return false;

    for (let i = 0; i < shape.points.length - 1; i++) {
      //@ts-ignore
      const { x: x1, y: y1 } = shape.points[i];
      //@ts-ignore
      const { x: x2, y: y2 } = shape.points[i + 1];

      if (this.isNearLine({ type: "line", x1, y1, x2, y2 }, x, y, threshold)) {
        return true;
      }
    }
    return false;
  ...
  eraseShape(x: number, y: number) {
    ...
    this.existingShapes = this.existingShapes.filter((shape) => {
      ...
      switch (shape.type) {
        ...
        case "rhombus":
          return !this.isPointInsideRhombus(x, y, shape);
        case "pencil":
  ...
  mouseDownHandler = (e: MouseEvent) => {
    ...
    if (this.selectedTool === "pencil") {
      ...
      for (let i = this.existingShapes.length - 1; i >= 0; i--) {
        ...
        if (
          (shape?.type === "line" && this.isNearLine(shape, x, y)) ||
          (shape?.type === "pencil" && this.isNearPencilStroke(shape, x, y))
        ) {
